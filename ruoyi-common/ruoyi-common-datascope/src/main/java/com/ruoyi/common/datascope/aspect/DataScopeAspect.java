...
public class DataScopeAspect
{
    ...
    public static void dataScopeFilter(JoinPoint joinPoint, SysUser user, String userAlias, String deptAlias, String userField, String deptField, String permission)
    {
        ...
        for (SysRole role : user.getRoles())
        {
            ...
            {
                sqlString.append(StringUtils.format(" OR {}.{} IN ( SELECT dept_id FROM sys_dept WHERE dept_id = {} or find_in_set({}, ancestors) > 0 )", deptAlias, deptField, user.getDeptId(), user.getDeptId()));
            }
